...
class KitchenDownStreamTask(SingleArmEnv):
    ...
    def reward(self, action=None):
        ...
        if self.stage == 0 and butter_in_pot:
            self.stage = 1
        ...
        # Stage 2 -> 3: Meatball placed in pot
        elif self.stage == 2 and meatball_in_pot:
            self.stage = 3

        # Stage 3 -> 4: Pot on stove
        elif self.stage == 3 and pot_on_stove:
            self.stage = 4

        # Stage 4 -> 5: Button pressed (stove on)
        elif self.stage == 4 and self.button_on:
            self.stage = 5

        # Stage 5 -> 6: Pot moved to target
        elif self.stage == 5 and pot_on_target:
            self.stage = 6

        # Stage 6 -> 7: Button turned off
        elif self.stage == 6 and not self.button_on:
            self.stage = 7

        # Stage 7 -> 8: Meatball cooked
        elif self.stage == 7 and self.meatball_cook_status == 1:
            self.stage = 8
    ...
    def step(self, action):
        ...
        return next_obs, self.stage, done, info
