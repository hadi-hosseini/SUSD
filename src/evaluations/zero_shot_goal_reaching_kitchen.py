...
if algo == "dsd":
    ...
    csv_path = "results/zero_shot_dsd_kitchen.csv"
...
def zero_shot_eval(env, max_duration=30.0, max_steps=200):
    ...
    while time.time() - start_time < max_duration:
        if done:
            ...
            steps = 0
            completed_tasks.append(info.get("episode_task_completions", []))
        ...
        while not done and steps < max_steps and (time.time() - start_time < max_duration):
            ...
            g_tensor = torch.from_numpy(goal).float().unsqueeze(0).to(device)
    ...
    return time_reward_log, cumulative_reward, completed_tasks, number_of_completed_tasks
...
time_reward_log, total_reward, completed_tasks, number_of_completed_tasks = zero_shot_eval(env, max_duration=30, max_steps=200)
print(f"Total reward: {total_reward:.2f}")
print(f"Number of completed tasks: {number_of_completed_tasks}")
print(f"Completed tasks: {completed_tasks}")
print(time_reward_log)
